...
export default function EditHackathonPage() {
  ...
  useLayoutEffect(() => {
    ...
    setApplyDeadline(
      hackathon.applyDeadline ? new Date(hackathon.applyDeadline).toISOString().slice(0, 16) : ''
    )
    ...
    setScoringDeadline(
      hackathon.scoringDeadline ? new Date(hackathon.scoringDeadline).toISOString().slice(0, 16) : ''
    )
    const mode =
  ...
  if (isError && error) {
    ...
    return (
      <div>
        <PageHeader title="Edit challenge" description="Error loading challenge.">
  ...
  return (
    <div>
      <PageHeader title="Edit challenge" description={`Update ${hackathon.title}.`}>
        <Button variant="outline" asChild>
          ...
          >
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium" htmlFor="image">
                Banner image (optional, 5:3, max 2 MB). Leave empty to keep the current image.
              </label>
              <Input
                id="image"
                type="file"
