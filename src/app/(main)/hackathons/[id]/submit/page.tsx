...
import { getCurrentDailyDayNumber, getInstructionForDay } from '@/lib/hackathon-deadlines'
...
export default function HackathonSubmitPage() {
  ...
  const { data: thread, isLoading: threadLoading } = useMySubmissionThread(id || null)
  ...
  const sortedThreadEntries = [...threadEntries].sort((a, b) =>
    new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  )
  const hasFinalSubmitted = Boolean(participation?.hasSubmitted)

  const hasSubmittedToday = !hasFinalSubmitted && Boolean(
    threadEntries.some((entry) => {
      const submittedDay = new Date(entry.entryDate).toISOString().slice(0, 10)
  ...
  useEffect(() => {
    return () => {
      ...
      if (finalSessionId) {
        void abortFinalSubmissionChunkUpload(finalSessionId).catch(() => undefined)
      }
    }
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const chunkSize = Math.min(Math.max(DEFAULT_CHUNK_SIZE, 5 * 1024 * 1024), 10 * 1024 * 1024)
      ...
      for (let chunkIndex = 0; chunkIndex < computedTotalChunks; chunkIndex++) {
        ...
        const progress = await uploadDailyThreadChunk(init.id, chunkIndex, chunkBlob)
      ...
      queryClient.invalidateQueries({ queryKey: ['submission-threads-hackathon', id] })
      setFeedbackMessage('')
      setFile(null)
      resetUploadProgress()
    } catch (err) {
      ...
      const messageText = err instanceof Error ? err.message : 'Chunked upload failed'
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let chunkIndex = 0; chunkIndex < computedTotalChunks; chunkIndex++) {
        ...
        const progress = await uploadFinalSubmissionChunk(init.id, chunkIndex, chunkBlob)
      ...
      queryClient.invalidateQueries({ queryKey: ['submission-thread-me', id] })
      queryClient.invalidateQueries({ queryKey: ['participation', 'hackathon', id] })
      queryClient.invalidateQueries({ queryKey: ['submissions'] })
      setFinalTitle('')
      setFinalDescription('')
      setFinalFile(null)
      ...
      const messageText = err instanceof Error ? err.message : 'Failed to create final submission'
  ...
  const dailyInstructionToday =
    dailyDayNumber != null ? getInstructionForDay(hackathon.dailyInstructions, dailyDayNumber) : null
      ...
      <div className="mx-auto max-w-3xl">
        {isDailyMode ? (
          <div className="space-y-6">
            <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-cs-border bg-card p-4">
              <h3 className="font-medium text-cs-heading">Post today&apos;s update</h3>
              {dailyDayNumber === null ? (
                <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-800 dark:text-amber-200">
                  Daily updates are only available from the first UTC day after the apply deadline through
                ...
                <div className="rounded-md border border-cs-primary/25 bg-cs-primary/5 p-3 text-sm">
                  <p className="font-medium text-cs-heading">Day {dailyDayNumber}</p>
                  <p className="mt-1 whitespace-pre-wrap text-muted-foreground">{dailyInstructionToday}</p>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">
                  No organizer instruction is set for day {dailyDayNumber}; you can still post your update.
                </p>
              )}
              <div>
                <label htmlFor="submit-feedback" className="mb-1.5 block text-sm font-medium text-cs-heading">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  One update per UTC day until the final submission deadline ({new Date(hackathon.finalSubmissionDeadline).toLocaleString()}).
                </p>
              </div>
              {hasSubmittedToday ? (
                <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-700 dark:text-amber-300">
                  Today&apos;s update is already posted. Come back tomorrow for the next update.
                </p>
              ) : null}
              {isUploading ? (
                <div className="space-y-2 rounded-md border border-cs-border bg-muted/30 p-3">
                  <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                    <div
                      ...
                      style={{ width: `${Math.max(0, Math.min(uploadPercent, 100))}%` }}
                    />
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Uploading {uploadPercent}% ({currentChunk}/{totalChunks} chunks)
                  </p>
                </div>
              ) : null}
              {uploadError ? (
                <p className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                  Upload failed and partial data was cleaned up. Retry upload. ({uploadError})
                </p>
              ) : null}
              <div className="flex gap-2">
                <Button type="submit" disabled={isUploading || !file || dailyFormDisabled}>
            ...
            <div className="rounded-lg border border-cs-border bg-card p-4">
              <h3 className="mb-4 font-medium text-cs-heading">Submission thread</h3>
              {sortedThreadEntries.length === 0 ? (
                ...
                <ol className="relative m-0 list-none space-y-5 p-0">
                  ...
                  {sortedThreadEntries.map((entry, idx) => {
                    ...
                    return (
                      <li key={entry.id} className="relative grid grid-cols-[1.5rem_minmax(0,1fr)] items-start gap-4">
                        <span
                        ...
                        >
                          {isLatest ? <CheckCircle2 className="size-3.5" /> : <span className="size-2 rounded-full bg-current" />}
                        </span>
                        <div className="rounded-md border border-cs-border bg-muted/20 p-3">
                          <p className="text-xs text-muted-foreground">
                            {new Date(entry.createdAt).toLocaleString()}
                          </p>
                          <p className="mt-1 text-sm font-semibold text-cs-heading">{title}</p>
                          {body ? (
                            <p className="mt-1 whitespace-pre-wrap text-sm text-cs-text">{body}</p>
                          ) : null}
                          <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                            ...
                            >
                              ...
                              {downloadingEntryId === entry.id ? 'Downloading...' : 'Download'}
                            </Button>
                          </div>
                        </div>
                      </li>
          ...
          <form onSubmit={handleFinalSubmit} className="space-y-4 rounded-lg border border-cs-border bg-card p-4">
            <h3 className="font-medium text-cs-heading">Final submission</h3>
            <div>
              <label htmlFor="final-title" className="mb-1.5 block text-sm font-medium text-cs-heading">
                Title
              </label>
              <Input
                id="final-title"
            ...
            <div>
              <label htmlFor="final-desc" className="mb-1.5 block text-sm font-medium text-cs-heading">
                Description
              </label>
              <textarea
                id="final-desc"
            ...
            <div>
              <label htmlFor="final-file" className="mb-1.5 block text-sm font-medium text-cs-heading">
                Final file
              </label>
              <Input
                id="final-file"
            ...
            {submittingFinal ? (
              <div className="space-y-2 rounded-md border border-cs-border bg-muted/30 p-3">
                <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                  <div
                    className="h-full bg-primary transition-all"
                    style={{ width: `${Math.max(0, Math.min(finalUploadPercent, 100))}%` }}
                  />
                </div>
                <p className="text-xs text-muted-foreground">
                  Uploading final file {finalUploadPercent}% ({finalCurrentChunk}/{finalTotalChunks} chunks)
                </p>
              </div>
            ) : null}
            {finalUploadError ? (
              <p className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                Final upload failed and partial data was cleaned up. Retry upload. ({finalUploadError})
              </p>
            ) : null}
            <div className="flex items-center gap-2">
              <Button type="submit" disabled={submittingFinal || hasFinalSubmitted || !finalFile}>
