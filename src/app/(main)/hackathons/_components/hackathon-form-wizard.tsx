...
export const HACKATHON_FORM_STEP_LABELS_COMPACT: Record<HackathonFormStepId, string> = {
  basics: 'Basics',
  timeline: 'Schedule',
  daily: 'Daily',
  rules: 'Rules',
...
export function HackathonFormStepIndicator({ steps, currentIndex }: StepIndicatorProps) {
      ...
      <div className="sm:hidden">
            ...
            return (
              <span
                ...
                className={cn(
                  ...
                  active && 'h-2 bg-primary shadow-sm ring-2 ring-primary/30 ring-offset-2 ring-offset-background',
        ...
        <ol className="flex flex-wrap justify-center gap-x-3 gap-y-3">
          {steps.map((id, i) => {
            ...
            const active = i === safeIndex
            return (
              <li key={id} className="flex w-19 shrink-0 flex-col items-center gap-1 text-center">
                <span
                  className={cn(
                    ...
                    !done && !active && 'border-cs-border bg-card text-muted-foreground'
                  )}
...
export function HackathonFormStepPanel({ title, description, children, className }: StepPanelProps) {
  return (
    <div
      className={cn(
