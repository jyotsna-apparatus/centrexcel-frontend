...
const ResetPasswordPage = () => {
  ...
  const verifyOtpMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        data.data.message ?? "Code verified. Set your new password.",
      );
      setStep(2);
    },
    onError: (error: Error) => {
  ...
  const handleStep2 = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!resetToken) {
      toast.error(
        "Session expired. Please request a new code from the forgot password page.",
      );
      setStep(1);
      return;
    }
            ...
            <form onSubmit={handleStep1} className="flex flex-col gap-4 w-full">
              ...
              {step1.email && (
                <button
                  ...
                  disabled={
                    resendMutation.isPending || resendCooldownSeconds > 0
                  }
