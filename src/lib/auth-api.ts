...
const getBaseUrl = (): string => {
  ...
  const base =
    raw?.replace(/\/$/, "") ?? DEFAULT_BACKEND_BASE_URL.replace(/\/$/, "");
...
export type LoginUser = {
  ...
  /** When not true, user must complete onboarding before browsing hackathons */
  isOnboarded?: boolean;
...
/** Login succeeded but email is not verified — OTP was sent; redirect to verify-email. */
export type LoginNeedsVerificationResult = {
  success: true;
...
export type LoginResult = LoginResponse | LoginNeedsVerificationResult;
...
export async function login(
  ...
  const payload = (
    json as { data?: { needsEmailVerification?: boolean; email?: string } }
  ).data;
  if (
    payload?.needsEmailVerification === true &&
  ...
  const accessToken =
    (json as LoginResponse).data?.accessToken ??
    (json as { accessToken?: string }).accessToken;
  const refreshToken =
    (json as LoginResponse).data?.refreshToken ??
    (json as { refreshToken?: string }).refreshToken;
  const user =
    (json as LoginResponse).data?.user ?? (json as { user?: LoginUser }).user;
  if (!accessToken || !user) {
    throw new Error(
      (json as { message?: string }).message ?? "Invalid login response",
    );
  }
  return {
    success: true,
    data: {
      ...
      user: {
        ...user,
        id:
          (user as LoginUser & { id?: string }).id ??
...
export async function getMe(accessToken?: string | null): Promise<MeResponse> {
  ...
  const res =
    accessToken != null
      ...
      : await authenticatedFetch(`${baseUrl}/auth/me`, {
          method: "GET",
          headers: { accept: "*/*" },
        });
  ...
  if (!rawUser) {
    throw new Error(
      (json as { message?: string }).message ?? "Invalid response",
    );
  }
  const user = {
    ...rawUser,
    id:
      (rawUser as { userId?: string }).userId ??
...
export async function checkUsername(
  ...
  const res = await authFetch(
    `${baseUrl}/auth/check-username/${encodeURIComponent(trimmed)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
  );
  const json = (await res.json()) as CheckUsernameResponse & {
    message?: string;
    data?: { available?: boolean };
  };
  if (!res.ok) {
...
export async function resendOtp(
  ...
  const json = (await res.json()) as ResendOtpResponse & {
    message?: string;
    error?: string;
  };
  if (!res.ok) {
  ...
  return {
    success: true,
    message: (json as { message?: string })?.message ?? "OTP sent",
  };
}
...
export async function verifyResetOtp(
  ...
  const json = (await res.json()) as VerifyResetOtpResponse & {
    message?: string;
    error?: string;
  ...
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Invalid or expired OTP");
    throw new Error(message);
  }
...
export async function resetPassword(
  ...
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Reset failed");
    throw new Error(message);
  }
  return { success: true, message: json?.message, data: json?.data };
}
...
export async function changePassword(
  ...
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Change failed");
    throw new Error(message);
  }
  return {
    success: true,
...
export async function getUser(id: string): Promise<UserListItem> {
  ...
  if (!res.ok) {
    throw new Error(
      typeof json?.message === "string" ? json.message : "Failed to fetch user",
    );
  }
...
export async function updateProfile(
  ...
  const json = (await res.json()) as UpdateProfileResponse & {
    data?: LoginUser;
    message?: string;
  };
  if (!res.ok) {
...
export async function updateProfilePicture(
  ...
  const res = await authenticatedFetch(`${baseUrl}/auth/me/profile-picture`, {
    method: "POST",
    body: form,
  });
  const json = (await res.json()) as OnboardingCompleteResponse & {
...
export async function requestEmailChange(
  ...
  const json = (await res.json()) as RequestEmailChangeResponse & {
    message?: string;
  };
  if (!res.ok)
    throw new Error(
      typeof json?.message === "string" ? json.message : "Failed to send code",
    );
...
export async function confirmEmailChange(
  ...
  const json = (await res.json()) as ConfirmEmailChangeResponse & {
    message?: string;
  };
  if (!res.ok)
    throw new Error(
...
export async function requestPasswordChangeOtp(): Promise<RequestPasswordChangeOtpResponse> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/auth/request-password-change-otp`,
    {
      method: "POST",
      headers: { accept: "application/json" },
    },
  );
  const json = (await res.json()) as RequestPasswordChangeOtpResponse & {
    message?: string;
  };
  if (!res.ok)
    throw new Error(
...
export async function confirmPasswordChange(
  body: ConfirmPasswordChangeBody,
): Promise<ConfirmPasswordChangeResponse> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/auth/confirm-password-change`,
    {
      method: "POST",
      headers: {
        accept: "application/json",
  ...
  const json = (await res.json()) as ConfirmPasswordChangeResponse & {
    message?: string;
  };
  if (!res.ok)
    throw new Error(
...
export async function createFavorite(
  ...
  const json = (await res.json()) as CreateFavoriteResponse & {
    message?: string;
    error?: string;
  };
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Failed to create favorite");
    throw new Error(message);
  }
...
export async function getFavorites(
  ...
  const json = (await res.json()) as GetFavoritesResponse & {
    message?: string;
    error?: string;
  };
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Failed to fetch favorites");
    throw new Error(message);
  }
...
export async function deleteFavorite(favoriteId: string): Promise<void> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/favorites/${encodeURIComponent(favoriteId)}`,
    {
      method: "DELETE",
      headers: { accept: "application/json" },
    },
...
export async function getTeam(id: string): Promise<Team> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/teams/${encodeURIComponent(id)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
  ...
  if (!res.ok) {
    throw new Error(
      typeof json?.message === "string" ? json.message : "Failed to fetch team",
    );
  }
...
export async function createTeam(
  ...
  const json = (await res.json()) as CreateTeamResponse & {
    message?: string;
    error?: string;
  };
  if (!res.ok) {
    const message =
      typeof json?.message === "string"
        ? json.message
        : ((json as { error?: string })?.error ?? "Failed to create team");
    throw new Error(message);
  }
...
export async function getFeaturedHackathons(
  ...
  if (!res.ok) {
    throw new Error(
      typeof json?.message === "string"
        ? json.message
        : "Failed to fetch featured challenges",
    );
  }
  return Array.isArray(json.data) ? json.data : [];
}
...
export async function getHackathons(params: {
  ...
  const json = (await res.json()) as {
    ...
    data?:
      ...
      | {
          data?: HackathonListItem[];
          pagination?: GetHackathonsResponse["pagination"];
        };
  };
  if (!res.ok) {
    throw new Error(
...
export async function getSubmissions(params: {
  ...
  const json = (await res.json()) as {
    ...
    data?:
      ...
      | {
          data?: SubmissionListItem[];
          pagination?: GetSubmissionsResponse["pagination"];
        };
  };
  if (!res.ok) {
    throw new Error(
...
export async function getHackathon(id: string): Promise<Challenge> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/hackathons/${encodeURIComponent(id)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function createHackathon(
  form: CreateHackathonFormData,
): Promise<{ success: boolean; message: string; data: Challenge }> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  ...
  if (form.dailyInstructionsJson)
    body.append("dailyInstructions", form.dailyInstructionsJson);
  ...
  if (form.isPaid && form.priceOfEntry != null)
    body.append("priceOfEntry", String(form.priceOfEntry));
...
export type UpdateHackathonFormData = Partial<CreateHackathonFormData> & {
  status?: string;
};
...
export async function updateHackathon(
  ...
  if (form.shortDescription !== undefined)
    body.append("shortDescription", form.shortDescription);
  if (form.applyDeadline !== undefined)
    body.append("applyDeadline", form.applyDeadline);
  if (form.finalSubmissionDeadline !== undefined)
    body.append("finalSubmissionDeadline", form.finalSubmissionDeadline);
  if (form.scoringDeadline !== undefined)
    body.append("scoringDeadline", form.scoringDeadline);
  if (form.submissionMode !== undefined)
    body.append("submissionMode", form.submissionMode);
  if (form.dailyInstructionsJson !== undefined)
    body.append("dailyInstructions", form.dailyInstructionsJson);
  if (form.instructions !== undefined)
    body.append("instructions", form.instructions);
  if (form.sponsorId !== undefined) body.append("sponsorId", form.sponsorId);
  if (form.judgeIds !== undefined)
    body.append("judgeIds", JSON.stringify(form.judgeIds));
  if (form.isPaid !== undefined)
    body.append("isPaid", form.isPaid ? "true" : "false");
  if (form.priceOfEntry !== undefined && form.priceOfEntry != null)
    body.append("priceOfEntry", String(form.priceOfEntry));
...
/** Admin: approve, reject, or request changes on a sponsor-submitted hackathon. */
export async function reviewHackathon(
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/hackathons/${encodeURIComponent(id)}/approval`,
    {
      method: "PATCH",
      headers: {
        accept: "application/json",
...
/** Delete hackathon (admin any; sponsor only while not yet approved) */
...
export async function getHackathonEntries(
  hackathonId: string,
): Promise<Submission[]> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/hackathons/${encodeURIComponent(hackathonId)}/entries`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function downloadHackathonEntries(
  hackathonId: string,
): Promise<Blob> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/hackathons/${encodeURIComponent(hackathonId)}/download-entries`,
    {
      method: "GET",
...
export async function requestTeamDeletion(
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/teams/${encodeURIComponent(teamId)}/request-deletion`,
    {
      method: "POST",
      headers: { accept: "application/json" },
    },
  ...
  return {
    success: true,
    message: json?.data?.message ?? json?.message ?? "Deletion requested",
  };
}

/** Confirm team deletion (any member). When all confirm, team is dissolved. */
export async function confirmTeamDeletion(
  teamId: string,
): Promise<{ success: boolean; message: string }> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/teams/${encodeURIComponent(teamId)}/confirm-deletion`,
    {
      method: "POST",
      headers: { accept: "application/json" },
    },
  ...
  return {
    success: true,
    message: json?.data?.message ?? json?.message ?? "Deletion confirmed",
  };
}
...
export async function getTeamDeletionStatus(
  teamId: string,
): Promise<TeamDeletionStatus> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/teams/${encodeURIComponent(teamId)}/deletion-status`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function getMyParticipations(params?: {
  ...
  const res = await authenticatedFetch(
    `${baseUrl}/participations?page=${page}&limit=${limit}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
  ...
  const json = (await res.json()) as {
    data?: GetParticipationsResponse;
    message?: string;
  };
  if (!res.ok) {
    throw new Error(
...
export async function getParticipationForHackathon(
  ...
  const json = (await res.json()) as {
    data?: ParticipationListItem | null;
    message?: string;
  };
  if (!res.ok) {
    throw new Error(
...
export async function getHackathonParticipations(
  ...
  const json = (await res.json()) as {
    data?: HackathonParticipationAdmin[];
    message?: string;
  };
  if (!res.ok) {
    throw new Error(
...
export async function getSubmission(id: string): Promise<Submission> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/submissions/${encodeURIComponent(id)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function getSubmissionsByHackathon(
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/submissions/hackathon/${encodeURIComponent(hackathonId)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function createDailyThreadEntry(
  ...
  body.append("file", form.file);
  const res = await authenticatedFetch(
    `${baseUrl}/submissions/thread/entries`,
    {
      method: "POST",
      headers: { accept: "application/json" },
      body,
...
export async function initDailyThreadChunkUpload(form: {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/submissions/thread/uploads/init`,
    {
      method: "POST",
      headers: {
        accept: "application/json",
...
export async function abortDailyThreadChunkUpload(
  sessionId: string,
): Promise<void> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
...
export async function initFinalSubmissionChunkUpload(form: {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/submissions/final/uploads/init`,
    {
      method: "POST",
      headers: {
        accept: "application/json",
...
export async function abortFinalSubmissionChunkUpload(
  sessionId: string,
): Promise<void> {
  const baseUrl = getBaseUrl();
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
...
export async function getMySubmissionThread(
  hackathonId: string,
  ...
  if (!res.ok) {
    throw new Error(
      typeof json?.message === "string"
        ? json.message
        : "Failed to fetch submission thread",
    );
  }
  return json?.data ?? null;
}
...
export async function getHackathonSubmissionThreads(
  hackathonId: string,
  ...
  if (!res.ok) {
    throw new Error(
      typeof json?.message === "string"
        ? json.message
        : "Failed to fetch thread updates",
    );
  }
  return Array.isArray(json?.data) ? json.data : [];
}
...
export async function downloadThreadEntry(
  entryId: string,
  filename?: string,
): Promise<void> {
  const baseUrl = getBaseUrl();
...
export async function getSubmissionScores(
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/scores/submission/${encodeURIComponent(submissionId)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
...
export async function updateScore(body: UpdateScoreBody): Promise<JudgeScore> {
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/scores/${encodeURIComponent(body.scoreId)}`,
    {
      method: "PATCH",
      headers: {
        accept: "application/json",
...
export async function getHackathonWinners(
  ...
  if (!baseUrl) throw new Error("NEXT_PUBLIC_BACKEND_BASE_URL is not set");
  const res = await authenticatedFetch(
    `${baseUrl}/winners/hackathon/${encodeURIComponent(hackathonId)}`,
    {
      method: "GET",
      headers: { accept: "application/json" },
    },
